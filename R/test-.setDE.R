test_that(".setDE: scalar deLogFC, 1 group, 2 timepoints", {
    ...
    expect_length(result$deFacs, 3)
    expect_true(all(result$deFacs %in% c(1, -1)))  # since only +/-1 possible
})
...
test_that(".setDE: vector deLogFC, propDE < 1", {
    ...
    expect_true(any(result$deFacs == 0))
    expect_true(all(result$deFacs %in%c(0,1,-1)))
})
...
test_that(".setDE: returns all 1s if propDE = 0", {
    ...
    expect_true(all(result$deFacs == 0))
})
