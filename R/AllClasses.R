...
#' Must be a value >=0
#' @slot customSampleFacVar An optional numeric vector of gene-specific variance
#' values for sample-level multiplicative factors. Values must be >=0.
#' When not provided, variance values are drawn from a distribution using
#' \code{sampleFacVarMean} and \code{sampleFacVarSD} as parameters
...
#' Must be a value >=0.
#' @slot customSubjectFacVar An optional numeric vector of gene-specific variance
#' values for subject-level multiplicative factors. Values must be >=0.
#' When not provided, variance values are drawn from a distribution using
#' \code{subjectFacVarMean} and \code{subjectFacVarSD} as parameters
...
.checkDeLog2FCFormat <- function(deLog2FC, nTimepoints, twoGroupDesign) {
    ...
    if (is.list(deLog2FC)) {
        ...
        if (nTimepoints == 1 && twoGroupDesign) {
            ...
            expected_names<-expected_names[expected_names!=forbidden_name]
