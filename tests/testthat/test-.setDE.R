test_that(".setDE: scalar deLog2FC, 1 group, 2 timepoints", {
    colDat <- data.frame(group = rep(0, 4), time = rep(c(0, 1), each = 2))
    result <- rescueSim:::.setDE(deLog2FC = 1, propDE = 1, nGenes = 3, colDat = colDat)
...
test_that(".setDE: vector deLog2FC, propDE < 1", {
    colDat <- data.frame(group = rep(0, 4), time = rep(c(0, 1), each = 2))
    set.seed(24)
    result <- rescueSim:::.setDE(deLog2FC = c(-1, 1), propDE = 0.5, nGenes = 4, colDat = colDat)
...
test_that(".setDE: list deLog2FC, one timepoint, two groups", {
    colDat <- data.frame(group = rep(0:1, each = 2), time = rep(0, 4))
    custom_deLog2FC <- list(group1 = c(1, 0, 0))

    result <- rescueSim:::.setDE(deLog2FC = custom_deLog2FC, propDE = numeric(0), nGenes = 3, colDat = colDat)
...
test_that(".setDE: list deLog2FC, two timepoints, one group", {
    colDat <- data.frame(group = rep(0, 4), time = rep(0:1, each = 2))
    geneNames <- c("geneA", "geneB", "geneC")
    custom_deLog2FC <- list(time1 = setNames(c(0, 1, 0), geneNames))
...
test_that(".setDE: list deLog2FC, two timepoints and two groups", {
    colDat <- expand.grid(time = 0:1, group = 0:1)
    geneNames <- c("geneA", "geneB", "geneC")
    custom_deLog2FC <- list(
...
test_that(".setDE: returns all 1s if propDE = 0", {
    colDat <- data.frame(group = rep(0, 4), time = rep(c(0, 1), each = 2))
    result <-rescueSim:::.setDE(deLog2FC = 1, propDE = 0, nGenes = 3, colDat = colDat)

    expect_true(all(result$de == 1))
    expect_true(all(result$deFacs[[1]] == 0))
})

test_that(".setDE: list deLog2FC, 3 timepoints, 1 group", {
    ...
    # Provide Log2FC for time1 and time2 vs. time0 (baseline)
    custom_deLog2FC <- list(
...
test_that(".setDE: numeric deLog2FC gives linear change over 3 timepoints, 1 group", {
    set.seed(12)
    colDat <- data.frame(
        group = rep(0, 6),
        time = rep(0:2, each = 2)
    ...
    result <- rescueSim:::.setDE(deLog2FC = 1, propDE = 1, nGenes = 1, colDat = colDat)

    # Reverse-engineer the design factor:
    # For group=0, function internally adds 1, so group=1
    # time / (nTimepoints - 1) * group = time / 2
    expected_Log2FC <- c(0, 0.5, 1)  # for time0, time1, time2
    expected_FC <- 2^expected_Log2FC
...
test_that(".setDE: numeric deLog2FC gives linear interaction over 3 timepoints, 2 groups", {
    set.seed(24)
    colDat <- expand.grid(
        time = 0:2,
        group = 0:1
    ...
    expected_Log2FC <- outer(0:2, 0:1, function(t, g) (t / 2) * g)*-1
    expected_FC <- 2^expected_Log2FC
